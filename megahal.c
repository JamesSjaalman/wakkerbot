...
STATIC WordNum ref_word_noindex(DICTIONARY *, STRING);
...
return *wp;
	...
	fprintf(stderr, "add_word( %*.*s ) dict->size=%u position=%d, found=%d\n"
		 , (int) word.length, (int) word.length, word.word, dict->size, position, found);
