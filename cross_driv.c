...
while (fgets(buff, sizeof buff, stdin)) {
	...
	case 'd': crosstab_show(stderr, cdp, format_num); continue;
	case '?': 
		cp += strcspn(cp, " \t\n");
		rc = sscanf( cp, "%u", &one);
		if (rc != 1) continue;
	...
	case 'h':
		...
		fprintf(stderr, "z <size>\t:= ResiZe\n" );
		fprintf(stderr, "r <size>\t:= Reduce to size\n" );
		fprintf(stderr, "l <name>\t:= Load file\n" );
